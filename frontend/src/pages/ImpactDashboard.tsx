...
import { Users, Home, CheckCircle, Clock, TrendingUp, Heart, ChevronLeft, ChevronRight } from 'lucide-react';
...
function SafiraLogo({ height = 24, color = '#000' }: { height?: number; color?: string }) {
  return (
    <svg width={height * 2.3} height={height} viewBox="0 0 92 40" fill="none" xmlns="http://www.w3.org/2000/svg">
...
function AreaSparkline({ data }: { data: DonationMonth[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-28" preserveAspectRatio="none">
      ...
      <path d={line} fill="none" stroke="#3b5ce5" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}

function SimpleBarChart({ data, valueKey, labelKey, color = '#3b5ce5' }: {
    ...
    <div className="flex items-end gap-1 h-36 w-full">
      {data.map((d, i) => (
        <div key={i} className="flex flex-col items-center gap-1 flex-1 h-full justify-end">
          <div className="w-full rounded-t-sm" style={{ height: `${max > 0 ? (values[i] / max) * 100 : 0}%`, background: color, minHeight: 2 }} />
          <span className="text-[10px] text-gray-400 truncate w-full text-center">{String(d[labelKey])}</span>
        </div>
      ))}
    </div>
  );
...
function GroupedBarChart({ data }: { data: OutcomeMonth[] }) {
  const maxVal = Math.max(...data.flatMap(d => [d.counselingSessions, d.educationMilestones, d.reintegrations * 10]));
  return (
      ...
      {data.map((d, i) => (
        <div key={i} className="flex flex-col items-center flex-1 h-full justify-end gap-0.5">
          <div className="flex items-end gap-px w-full">
            <div className="flex-1 rounded-t-sm" style={{ height: `${(d.counselingSessions / maxVal) * 140}px`, background: '#3b5ce5', minHeight: 2 }} />
            <div className="flex-1 rounded-t-sm" style={{ height: `${(d.educationMilestones / maxVal) * 140}px`, background: '#06b6d4', minHeight: 2 }} />
            <div className="flex-1 rounded-t-sm" style={{ height: `${((d.reintegrations * 10) / maxVal) * 140}px`, background: '#10b981', minHeight: 2 }} />
          </div>
...
function DonutRing({ pct, color = '#3b5ce5' }: { pct: number; color?: string }) {
  return (
...
function KpiCard({ value, label, icon }: { value: string | number; label: string; icon: ReactNode }) {
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-5 text-center">
      <div className="flex justify-center mb-2 text-[#3b5ce5]">{icon}</div>
      <div className="text-3xl font-extrabold text-[#3b5ce5] leading-none">{value}</div>
...
function StoryCarousel() {
  ...
  useEffect(() => {
    const t = setInterval(() => setIndex(i => (i + 1) % storySlides.length), 5000);
    return () => clearInterval(t);
  }, []);

  const prev = () => setIndex(i => (i - 1 + storySlides.length) % storySlides.length);
    ...
    <div className="bg-[#3b5ce5] text-white rounded-2xl overflow-hidden">
      <div className="px-8 py-7 min-h-[148px] flex flex-col justify-between">
        <div>
          ...
          <h3 className="text-lg font-bold leading-snug mb-2">{slide.headline}</h3>
      ...
      <div className="flex items-center justify-between px-8 pb-5">
        ...
        <div className="flex gap-2">
          <button onClick={prev} className="w-7 h-7 rounded-full bg-white/15 hover:bg-white/25 flex items-center justify-center transition-colors">
            <ChevronLeft size={14} />
          </button>
          <button onClick={next} className="w-7 h-7 rounded-full bg-white/15 hover:bg-white/25 flex items-center justify-center transition-colors">
...
export default function ImpactDashboard() {
  ...
  const totalResidents      = apiData?.totalResidents ?? safehouses.reduce((s, sh) => s + sh.residents, 0);
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900">
      ...
      <nav className="bg-white border-b border-gray-200 sticky top-0 z-50">
        <div className="max-w-5xl mx-auto px-8 h-14 flex items-center gap-8">
          <SafiraLogo height={28} color="#0f172a" />
          <div className="flex gap-1 flex-1 justify-center">
            {(['Home', 'About', 'Donate', 'Contact'] as const).map(l => (
              <a key={l} href={l === 'Home' ? '/' : '#'} className="px-3 py-1.5 rounded-md text-sm text-gray-500 hover:text-[#3b5ce5] hover:bg-blue-50 transition-colors">
                {l}
              </a>
            ))}
          </div>
          <button className="bg-[#3b5ce5] text-white text-sm font-semibold px-4 py-1.5 rounded-lg hover:bg-blue-700 transition-colors">
            Donate Now
          </button>
        </div>
      </nav>
      ...
      <div className="max-w-5xl mx-auto px-8 py-8 flex flex-col gap-6">
        ...
        <div className="grid grid-cols-4 gap-3">
          <KpiCard value={totalResidents}                             label="Girls in Our Care"         icon={<Users size={18} />} />
          <KpiCard value={totalReintegrations}                        label="Reintegrated This Year"     icon={<CheckCircle size={18} />} />
          <KpiCard value={totalSessions.toLocaleString()}             label="Counseling Sessions"        icon={<Heart size={18} />} />
          <KpiCard value={`$${(totalDonations / 1000).toFixed(0)}k`} label="Raised This Year"            icon={<TrendingUp size={18} />} />
        </div>
          ...
          <div className="p-5">
                ...
                <div className="flex-[1.6] flex flex-col gap-5">
                  <Card title="Donations over time">
                    ...
                    <div className="flex justify-between mt-1 px-0.5">
                      {donationTrends.filter((_, i) => i % 3 === 0).map((d, i) => (
                  ...
                  <Card title="Where your money goes">
                    {fundAllocation.map((item, i) => (
                      <div key={i} className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-xs font-bold w-7 text-right" style={{ color: item.color }}>{item.value}%</span>
                      </div>
                    ))}
                  </Card>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      {milestones.map((m, i) => (
                        <div key={i} className="flex flex-col items-center gap-2 text-center">
                          <DonutRing pct={m.pct} />
                          <span className="text-[11px] text-gray-500 leading-tight">{m.label}</span>
                        </div>
                      ))}
                    </div>
                  </Card>
            ...
            {tab === 'donations' && (
              <div className="flex flex-col gap-5">
                <div className="grid grid-cols-4 gap-3">
                  <KpiCard value={`$${(totalDonations / 1000).toFixed(0)}k`} label="Total Raised"         icon={<TrendingUp size={18} />} />
                  <KpiCard value="247"    label="Active Supporters"  icon={<Users size={18} />} />
                  <KpiCard value="82%"    label="Donor Retention"    icon={<Heart size={18} />} />
                  <KpiCard value="$1,120" label="Avg. Annual Gift"   icon={<CheckCircle size={18} />} />
                </div>
                <div className="flex gap-5">
                  <Card title="Monthly donations" className="flex-[1.6]">
                    <SimpleBarChart data={donationTrends as unknown as Record<string, number | string>[]} valueKey="amount" labelKey="month" color="#3b5ce5" />
                  </Card>
                  <Card title="Fund allocation" className="flex-1">
                    {fundAllocation.map((item, i) => (
                      <div key={i} className="flex items-center gap-2 mb-3.5">
                        <span className="w-2 h-2 rounded-sm shrink-0" style={{ background: item.color }} />
                        <span className="text-xs text-gray-600 w-36 shrink-0">{item.name}</span>
                        <div className="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                          <div className="h-full rounded-full" style={{ width: `${item.value * 2.4}%`, background: item.color }} />
                        </div>
                        <span className="text-xs font-bold w-7 text-right" style={{ color: item.color }}>{item.value}%</span>
                      </div>
                    ))}
            ...
            {tab === 'outcomes' && (
              <div className="flex flex-col gap-5">
                <div className="grid grid-cols-4 gap-3">
                  <KpiCard value={totalSessions.toLocaleString()} label="Counseling Sessions"  icon={<Heart size={18} />} />
                  <KpiCard value="172"                            label="Education Milestones"  icon={<CheckCircle size={18} />} />
                  <KpiCard value="98%"                            label="Health Checks Done"    icon={<Users size={18} />} />
                  <KpiCard value={totalReintegrations}            label="Reintegrations"        icon={<TrendingUp size={18} />} />
                </div>
                <div className="flex gap-5">
            ...
            {tab === 'safe homes' && (
              <div className="flex flex-col gap-5">
                <div className="grid grid-cols-4 gap-3">
                  <KpiCard value={activeSafehouses}            label="Active Safe Homes"  icon={<Home size={18} />} />
                  <KpiCard value={`${totalResidents} / 45`}    label="Occupancy"          icon={<Users size={18} />} />
                  <KpiCard value="11"                          label="Staff Members"       icon={<CheckCircle size={18} />} />
                  <KpiCard value="14 mo."                      label="Avg. Length of Stay" icon={<Clock size={18} />} />
                </div>
                  ...
                  {safehouses.map((sh, i) => {
                    ...
                    return (
                      <div key={i} className="bg-gray-50 border border-gray-200 rounded-xl p-5">
                        <h4 className="font-semibold text-gray-900 text-sm">{sh.name}</h4>
                        <p className="text-xs text-gray-400 mb-4">{sh.city}</p>
                        <div className="flex gap-2 mb-4">
                          {[
                            ...
                            <div key={j} className="flex-1 text-center">
                              <div className="text-xl font-extrabold text-[#3b5ce5]">{s.val}</div>
        ...
        <div className="bg-[#0f172a] text-white rounded-2xl px-8 py-8 text-center">
          ...
          <p className="text-sm text-slate-400 mb-6">Every dollar goes directly toward a child's safety and recovery.</p>
          <div className="flex gap-2 justify-center flex-wrap mb-4">
            {(['$25', '$50', '$100', '$250'] as const).map(amt => (
              <button key={amt} className="bg-white/10 border border-white/20 text-white text-sm font-semibold px-5 py-2 rounded-lg hover:bg-white/20 transition-colors">
                {amt}
              </button>
            ))}
          </div>
      ...
      {/* Footer */}
      <footer className="border-t border-gray-200 mt-6 py-6 text-center text-xs text-gray-400">
        &copy; 2026 Safira &nbsp;&middot;&nbsp; All data anonymized and aggregated &nbsp;&middot;&nbsp;
        <a href="/" className="hover:text-[#3b5ce5] transition-colors ml-1">Back to Home</a>
      </footer>
    </div>
  );
}
